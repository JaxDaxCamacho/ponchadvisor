...
// recebendo o nome do bar do ranking e guardando esse nome, para dps podermos
// mudar o nome para outro qualquer
if(isset($_POST['nome_bar'])){
    $nomeBar = $_POST['nome_bar'];
    $_SESSION['oldname']=$nomeBar;
    $stmt = $pdo->prepare("SELECT nome_bar,concelho,descricao FROM bar WHERE nome_bar LIKE ?");
    $stmt->execute(array($nomeBar));
    $barresult=$stmt->fetch();
...
if(isset($_POST["editado"])){
    ...
    $stmt = "UPDATE bar SET nome_bar= ?,concelho=? ,descricao=? WHERE nome_bar=?";
    $pdo->prepare($stmt) -> execute(array($nome,$localizacao,$descricao,$_SESSION['oldname']));
...
<div id="registo">
            ...
            <input type="text" name="nome" class="form-control" value="<?php echo$barresult['nome_bar']?>" aria-describedby="BarName" placeholder="Sem espaços">
            <small class="form-text text-muted">Se o nome do bar tiver caracteres especiais insira o equivalente</small>
        </div>
        <div class="form-group">
            <label >Localização</label>
            <input type="text" name="concelho" class="form-control" value="<?php echo$barresult['concelho']?>"  placeholder="Concelho/cidade">
        </div>
        <div class="form-group">
            <label >Descrição</label>
            <input type="text" name="descricao" class="form-control" value="<?php echo$barresult['descricao']?>"  placeholder="Mensagem rapida">
        </div>
        <div class="form-group">
